...
function getBrevoClient() {
  ...
  if (!apiKeyValue) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "BREVO_KEY is missing. Set it before deploying."
    );
  }
...
export const sendNewsletter = functions.https.onCall(async (data, context) => {
  ...
  if (!subject || !content) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "Campaign must have subject and content."
    );
  }
  ...
  try {
    const sendPromises = subscribers.map((subscriber) => {
      const unsubscribeUrl = `https://studio-8071626946-149cb.web.app/unsubscribe?email=${encodeURIComponent(
        subscriber.email
      )}`;
